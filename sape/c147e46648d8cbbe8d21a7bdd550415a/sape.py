...
class SapeBase:
    ...
    server_list = ['dispenser-01.saperu.net', 'dispenser-02.saperu.net']
    ...
    def __init__(self, options=None):
        ...
        self._use_server_array = True if options.get('use_server_array') else False

        # Какой сайт?
        self._host = host if host else SAPE_SITE

        self._host = self._host.replace('http://', '').replace('www.', '')

        # Какая страница?
        if options.get('request_uri'):
            self._request_uri = options.get('request_uri')
        elif not self._use_server_array:
            self._request_uri = getenv('REQUEST_URI')
        ...
        if options.get('multi_site'):
            self._multi_site = True
        ...
        # Сообщать об ошибках
        if options.get('verbose') or self._debug:
            self._verbose = True

        # Кодировка
        if options.get('charset'):
            self._charset = options.get('charset')
        else:
            self._charset = 'windows-1251'

        if options.get('fetch_remote_type'):
            self._fetch_remote_type = options.get('fetch_remote_type')

        if options.get('socket_timeout') && is_numeric($options['socket_timeout']) && options.get('socket_timeout') > 0:
            self._socket_timeout = options.get('socket_timeout')

        # Всегда выводить чек-код
        if options.get('force_show_code'):
            self._force_show_code = True
        ...
        if options.get('ignore_case'):
            ...
            self._request_uri = self._request_uri.lower()

        if options.get('show_counter_separately'):
            self._show_counter_separately = bool(options.get('show_counter_separately'))

        if options.get('format') and options.get('format') in ['serialize', 'php-require']:
