...
def configure():
    ...
    if cfgfile:
        ...
        if isinstance(cfgfile, str):
            defaults['basedir'] = os.path.abspath(os.path.dirname(os.path.dirname(cfgfile)))
        elif real_settings:
    ...
    global options
    try:
        platform = globals().get('platform')
    except AttributeError:
        pass
    try:
        formatters = globals().get('formatters')
    except AttributeError:
        pass
    if options is None:
        options = {}
...
def convert_request(request):
    ...
    request_uri = urllib.parse.quote(path) + (query_string and
            ('?' + query_string) or '')
